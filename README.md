...
| 实验结果 | 表格居中，优先展示平均值 ± 标准差，并加粗每个数据集的最佳方法结果 |
...
- 普通正文首行缩进两个汉字；表格整体居中，每个数据集的最佳结果整格加粗，并同时处理并列最佳。
...
它检查字体、OMML、正文首行缩进、表格居中与分页保护、百分比格式，以及正文中是否意外出现仓库路径、Git 身份或内部状态。最佳结果加粗涉及具体指标方向和原始未舍入值，仍需在逐页视觉检查时结合结果数据确认。
