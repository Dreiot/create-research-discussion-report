...
| 术语 | 中文优先；数据集、基准和方法等官方专名保留英文身份 |
| 数学公式 | 使用可编辑的 Word 原生 OMML |
| 正文排版 | 普通正文首行缩进两个汉字，标题、表格和公式等不缩进 |
| 实验结果 | 表格及全部单元格内容居中，单元格无首行缩进、段前段后间距并使用单倍行距；加粗每个数据集的最佳方法结果 |
...
- `Caltech101-7`、`Reuters-1200`、`UCI Mfeat`、`MHEALTH` 等数据集或基准专名保留官方英文写法。
- 普通正文首行缩进两个汉字；表格整体及全部单元格内容居中，单元格不首行缩进、段前段后均为 0，并使用单倍行距；每个数据集的最佳结果整格加粗，并同时处理并列最佳。
...
它检查字体、OMML、正文首行缩进、表格及单元格内容居中、表格段落零缩进、零段前段后间距与单倍行距、分页保护、百分比格式，以及正文中是否意外出现仓库路径、Git 身份或内部状态。最佳结果加粗涉及具体指标方向和原始未舍入值，仍需在逐页视觉检查时结合结果数据确认。
